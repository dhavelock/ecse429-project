...
@given('the task has a incomplete done status of "{doneStatus}"')
def step_impl(context, doneStatus):
    context.todo = create_todo({'id': context.id, 'doneStatus': doneStatus})

@given('the task has a complete done status of "{doneStatus}"')
def step_impl(context, doneStatus):
    context.todo = create_todo({'id': context.id, 'doneStatus': doneStatus})

@given('I request to edit an invalid task id in the system')
